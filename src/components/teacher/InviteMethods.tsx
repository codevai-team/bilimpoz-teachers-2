...
const InviteMethods: React.FC<InviteMethodsProps> = ({ referralLink }) => {
  ...
  return (
    <div className="bg-[var(--bg-card)] rounded-2xl p-4 sm:p-6 overflow-hidden">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button 
          onClick={handleTelegramShare}
          className="flex items-center gap-3 p-4 bg-[var(--bg-tertiary)] rounded-lg hover:bg-[var(--bg-hover)] transition-colors group"
        >
          <div className="p-2 bg-blue-500/10 rounded-lg group-hover:bg-blue-500/20 transition-colors flex-shrink-0">
            <svg className="h-5 w-5 text-blue-400" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="text-left flex-1 min-w-0">
            ...
            <p className="text-xs text-[var(--text-tertiary)] truncate">{getText('students.referral.telegramHint', 'Отправить в чат')}</p>
          </div>
        </button>
        ...
        >
          ...
          <div className="text-left flex-1 min-w-0">
            ...
            <p className="text-xs text-[var(--text-tertiary)] truncate">{getText('students.referral.whatsappHint', 'Поделиться ссылкой')}</p>
          </div>
        </button>
      </div>
    </div>
